---
name: advosy:advosy-context
description: |
  Advosy company structure, leadership, subsidiaries, contacts, and terminology.
  Use when the user mentions any Advosy subsidiary name, employee name, or company
  abbreviation like CF, PA, SA, VRZA, EOS, Claimsforce, Select Adjusters, Everest Air,
  Bloque, Pestkee, Southwest Packout, or Homeforce. Also use for contact info lookups.
user_invocable: true
invocation_hint: /advosy:context - Get Advosy company context (leadership, subsidiaries, contacts)
section: business
---

# Advosy Company Context

## Quick Reference

**What is Advosy?** A home services parent company in Arizona using EOS (Entrepreneurial Operating System).

**Core Principles:** Versatility, Growth, Win

**Subsidiaries:**
- **Select Adjusters** - Public Adjusting (PA)
- **VRZA** - Roofing Construction (new branding)
  - **Image Roofing** - Specialty roofs (under VRZA)
- **Advosy Construction** - Roofing (old branding, being replaced)
- **Everest Air** - HVAC and Plumbing
- **Bloque Restoration** - Restoration and Mitigation
- **Pestkee** - Pest Control (just launched)
- **Southwest Packout** - Packout Services

**Related but separate:** Panda Promo (Gary Fenn's side business)

---

## Leadership Team (EOS Structure)

The Leadership Team oversees ALL companies under Advosy.

| Seat | Name | Email | Phone | Notes |
|------|------|-------|-------|-------|
| **Visionary** | Brett Ray | brett@selectadjusters.com | 480-466-4472 | Founder, majority owner |
| **Integrator** | Gary Fenn | gary@advosy.com | 480-221-6959 | Lead developer, branding, owns Panda Promo |
| **Finance** | Wesley Hathcock | wesley@advosy.com | 480-528-2500 | Majority owner |
| **Sales** | Scott O'Dell | scott@advosyenergy.com | 480-213-2760 | Head of Sales, owns Sales L10 |
| **Marketing** | Chandler Ricks | chandler@advosy.com | 480-613-8024 | GoHighLevel specialist |
| **Operations** | Melissa Lunt | melissa@advosy.com | 480-613-8003 | Gary's sister, often remote from UT |
| **HR** | Kris Davis | kris@advosy.com | 480-613-8003 | |

---

## Sales L10 (Reports to Scott O'Dell)

Covers sales for ALL companies under Advosy.

| Seat | Name | Email | Phone | Scope |
|------|------|-------|-------|-------|
| **Head of D2D** | Brandon Cruz | Brandon@advosyenergy.com | 602-806-1486 | D2C for all companies |
| **Head of B2B** | Josh Williamson | josh@advosy.com | 480-400-9876 | Affiliates/referral partners |
| **Head of Construction Meetings** | Moises Morales | mo@selectadjusters.com | | Roofing now, expanding to other service lines |
| **Head of Everest Air Sales** | Neil Chrisney | neil@everestairllc.com | | Scott proposed "Head of Technician Sales" (pending) |
| **Head of Sales Recruiting & Training** | Sarah Wawerski | sarah@advosy.com | | New seat, not official yet |

---

## VRZA / Roofing Operations

| Role | Name | Email | Phone |
|------|------|-------|-------|
| **Operations Seat** | Brett Arrington | brett@advosyenergy.com | 480-613-8003 |
| **Head of Field Ops** | Alex Oyarzun | alex@advosyenergy.com | 623-233-2626 |
| **Head of Claim Ops** | Waylon Wood | waylon@selectadjusters.com | 480-613-8003 |
| **Head of AMs** | Chris Dimaculangan | chris@selectadjusters.com | 480-613-8003 |
| **Regional Manager** | Jonathan Gutierrez | jonathan@advosyenergy.com | 623-341-8184 |
| **Regional Manager** | Danny Murphy | danny@advosy.com | 602-730-3155 |

---

## Other Subsidiaries

### Select Adjusters (Public Adjusting)
| Role | Name | Email |
|------|------|-------|
| Head of Estimating | JC Carlisle | jc@selectadjusters.com |

### Everest Air (HVAC & Plumbing)
| Role | Name | Email |
|------|------|-------|
| Owner | Mark Brimley | mark@everestairllc.com |
| Operations | Luke Palmer | luke@everestairllc.com |
| Sales | Neil Chrisney | neil@everestairllc.com | <!-- Also on Sales L10 -->

### Bloque Restoration
| Role | Name | Email |
|------|------|-------|
| Co-Owner | Payson Dampt | payson@bloquerestoration.com |
| Co-Owner | Dale Fenn | dale@bloquerestoration.com |

### Southwest Packout
| Role | Name |
|------|------|
| Owner | Mack Linton |

### Pestkee (Pest Control)
| Role | Name | Notes |
|------|------|-------|
| Owner | Zach Batis | Previously Advosy Finance Seat |

---

## Key Support Roles

| Role | Name | Email | Notes |
|------|------|-------|-------|
| IT Support | Jeramy Hubbard | jeramy@advosy.com | Built Claimsforce & Homeforce |
| Employee Experience | Lisa LaToure | lisa@advosy.com | |

<!-- Mo Morales and Sarah Wawerski moved to Sales L10 above -->

---

## Key Systems

| System | Purpose | URL/Notes |
|--------|---------|-----------|
| **Claimsforce** | Claims CRM (Select Adjusters) | sa.claimsforce.net (rebranded EspoCRM) |
| **Homeforce** | Home services CRM | Rebranded EspoCRM |
| **GoHighLevel** | Marketing automation | Managed by Chandler |
| **n8n** | Workflow automation | Gary's: events.advosy.app |
| **RepCard** | D2D sales rep management | api.repcard.com |
| **RingCentral** | SMS/Phone | |

---

## Family Relationships

- **Gary Fenn** (Integrator) is brother to **Melissa Lunt** (Operations)
- **Dale Fenn** (Bloque co-owner) is cousin to Gary and Melissa

---

## Two Bretts Clarification

| Name | Role | Context |
|------|------|---------|
| **Brett Ray** | Visionary | Advosy founder, majority owner |
| **Brett Arrington** | VRZA Operations | Also Scott's partner for Bresco |

---

## Terminology

| Term | Meaning |
|------|---------|
| **EOS** | Entrepreneurial Operating System |
| **Seat** | EOS term for a defined role |
| **Supplementing** | Internal term for estimating (Select Adjusters) |
| **AM** | Account Manager |
| **CM** | Construction Meeting |
| **CF** | Claimsforce |
| **PA** | Public Adjuster/Adjusting |
| **D2D** | Door-to-door (sales) |
| **D2C** | Direct to consumer |
| **B2B** | Business to business |

---

## Bresco (Separate from Advosy)

**What:** Automation consulting side business
**Partners:** Scott O'Dell + Brett Arrington
**Focus:** n8n workflows, SurrealDB, custom automations
**Communication:** Telegram (not Slack, to keep separate from Advosy)

---

## n8n Instances

| Instance | Owner | Purpose | URL |
|----------|-------|---------|-----|
| Gary's | Gary Fenn | Production Advosy | events.advosy.app |
| Brett's | Brett Arrington | Bresco + prototypes | banc-r.app.n8n.cloud |

**Pattern:** Prototype in Brett's → Move to Gary's for production

---

## Advosy Tech Projects

| Project | Location | Status | Purpose |
|---------|----------|--------|---------|
| D2D Payroll | ~/Scott/claude-projects/d2d-payroll/ | ~90% complete | Payroll calculator for D2D reps |
| Spotio-CF | ~/Scott/claude-projects/spotio-cf/ | Code complete | Spotio → Claimsforce integration |
| D2D Apps | ~/Scott/claude-projects/d2d-apps/ | Prototype | React D2D apps (outlier, being replaced) |
| CF Automations | ~/Scott/claude-projects/automations/ | Active | n8n workflow automations |
| Advosy CRM | advosy.app | Active | Custom CRM (Gary's build, Nuxt 4 + Nuxt UI + SurrealDB) |

**Legacy v2 projects:** d2d-payroll and spotio-cf still on SurrealDB v2 + WASM SDK v1.3.2 (will migrate when consolidated into advosy-sales).

---

## Notion Database IDs (Legacy - Migrating to SurrealDB)

| Resource | Notion ID |
|----------|-----------|
| Advosy EOS Page | `2ec4b348-1256-800f-98d2-efa913d2ee91` |
| People Database | `2ee4b348-1256-8149-9f6d-f5a1c9501301` (117 records) |
| Software Systems Database | `2ee4b348-1256-8129-8d7f-d1c4c0e7846f` (17 records) |
| Organizations Database | `2ee4b348-1256-81c6-833a-c68df44434b0` (7 records) |

---

## Project Context for Advosy Work

When building tools for Advosy:
- **Users** are typically internal employees (adjusters, sales reps, managers)
- **Data** is often related to claims, customers, scheduling, or training
- **Claimsforce (EspoCRM)** is the existing CRM, new tools may integrate with it
- **Gary** will productionize anything that goes live, keep handoff in mind
- **n8n workflows** exist for operations, check Brett's instance for existing automations
- UI should feel corporate but approachable
- Data accuracy is critical for insurance/claims work
- Gary expects clean, well-documented code for productionization

---

## Full Resources

- **Employee Directory:** Google Sheet (requires authentication, ask Scott for access if needed)
- **Full Context Doc:** ~/Scott/growth-os/wiki/Work/Advosy/Corporate Structure.md
