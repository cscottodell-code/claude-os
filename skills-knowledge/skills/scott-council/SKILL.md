...
Read `references/council-protocol.md` for the full 6-phase protocol:
...
- Phase 6: Persist Verdict [AUTO] (ADR always; full transcript for major deliberations)

## Verdict Persistence (Two-Tier)

Every session produces an ADR in `wiki/global/ADR-NNN-<topic-slug>.md` —
auto-numbered, append-only history. Major deliberations (`--full`, multi-round, or
explicit archival request) ALSO get a full transcript directory at
`raw/sessions/<YYYY-MM-DD>-council-<topic-slug>/` containing each phase's output.
Quick triads = ADR only. See protocol Phase 6 for templates and trigger rules.
