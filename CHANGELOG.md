...
- **NEW `references/surrealdb-v3-agent-typescript.md`:** TypeScript code that bridges the SurrealQL patterns to a running agent. Covers: read context (vector+graph+memory in one query), call Claude (Anthropic SDK with adaptive thinking and streaming), extract entities (structured output with Zod), write results (atomic transaction), the full loop, and Nuxt API route integration. Uses Haiku for cheap entity extraction, Opus for reasoning.
